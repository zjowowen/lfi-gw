...
import lfigw.ffjord.lib.layers.wrappers.cnf_regularization as reg_lib
import lfigw.ffjord.lib.spectral_norm as spectral_norm
import lfigw.ffjord.lib.layers as layers
from lfigw.ffjord.lib.layers.odefunc import divergence_bf, divergence_approx
...
def build_condition_model_tabular(args, input_dim, context_dim, regularization_fns=None):

    hidden_dims = tuple(map(int, args.dims.split("-")))

    def build_cnf():
        diffeq = layers.ODEnet_conditioned(
            hidden_dims=hidden_dims,
    ...
    if args.batch_norm:
        # TODO
        bn_layers = [layers.MovingBatchNorm1d(input_dim, bn_lag=args.bn_lag) for _ in range(args.num_blocks)]
        bn_chain = [layers.MovingBatchNorm1d(input_dim, bn_lag=args.bn_lag)]
        for a, b in zip(chain, bn_layers):
            bn_chain.append(a)
            bn_chain.append(b)
        chain = bn_chain
