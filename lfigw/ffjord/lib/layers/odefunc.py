...
__all__ = ["ODEnet", "ODEnet_conditioned", "AutoencoderDiffEqNet", "ODEfunc", "AutoencoderODEfunc"]
...
class ODEnet_conditioned(nn.Module):
    """
    Helper class to make neural nets for use in conditioned continuous normalizing flows
    """

    def __init__(
        self, hidden_dims, input_shape, context_shape, strides, conv, layer_type="concat", nonlinearity="softplus", num_squeeze=0
    ):
        super(ODEnet_conditioned, self).__init__()
        self.num_squeeze = num_squeeze
        if conv:
            assert len(strides) == len(hidden_dims) + 1
            base_layer = {
            ...
            strides = [None] * (len(hidden_dims) + 1)
            base_layer = {
                "ignore": diffeq_layers.basic_conditioned.IgnoreLinear,
                "hyper": diffeq_layers.basic_conditioned.HyperLinear,
        ...
        for dim_out, stride in zip(hidden_dims + (input_shape[0],), strides):
            ...
            layer = base_layer(hidden_shape[0], dim_out, context_shape, **layer_kwargs)
            layers.append(layer)
            activation_fns.append(NONLINEARITIES[nonlinearity])
    ...
    def forward(self, t, context, y):
        dx = y
        # squeeze
        for _ in range(self.num_squeeze):
            dx = squeeze(dx, 2)
        for l, layer in enumerate(self.layers):
            dx = layer(t, context, dx)
            # if not last layer, use nonlinearity
            if l < len(self.layers) - 1:
                dx = self.activation_fns[l](dx)
        # unsqueeze
