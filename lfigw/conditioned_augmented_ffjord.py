...
class train_augmented_ffjord_model:
    def __init__(self):
    ...
    def train_epoch(self, flow, train_loader, optimizer, epoch,
        ...
        for batch_idx, (h, x, w, snr) in enumerate(train_loader):
            ...
            # generate augmented_dim gaussian noise
            aug_noise = torch.randn(x.shape[0], self.augmented_dim)
            ...
            if device is not None:
                ...
                aug_noise = aug_noise.to(device, non_blocking=True)

            # get the log probability of the noise
            logp_noise = standard_normal_logprob(aug_noise).sum(1, keepdim=True)

            y = h

            # concatenate noise to h
            x_noise = torch.cat((x, aug_noise), dim=1)

            # Compute log prob
            zero = torch.zeros(x_noise.shape[0], 1).to(device)

            # transform to z
            z, delta_logp = flow(x_noise, y, zero)

            # compute log q(z)
            logpz = standard_normal_logprob(z).sum(1, keepdim=True)

            logpx_noise = logpz - delta_logp

            logpx= logpx_noise - logp_noise

            loss = -logpx

            loss = loss.squeeze(-1)

            neg_logpx_noise= -logpx_noise.squeeze(-1)
            ...
            loss = (w * loss).mean()
            neg_logpx_noise = (w * neg_logpx_noise).mean()
            ...
            if (output_freq is not None) and (batch_idx % output_freq == 0):
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.4f}\tNeg_logpx_noise: {:.4f}'.format(
                    epoch, batch_idx *
                    train_loader.batch_size, len(train_loader.dataset),
                    100. * batch_idx / len(train_loader),
                    loss.item(), neg_logpx_noise.item()))
    ...
    def test_epoch(self, flow, test_loader, device=None):
        ...
        with torch.no_grad():
            ...
            for h, x, w, snr in test_loader:
                ...
                zero = torch.zeros(x_noise.shape[0], 1).to(x)
...
def obtain_samples(flow, y, nsamples, augmented_dim=1, device=None, batch_size=512):
    ...
    with torch.no_grad():
        ...
        p_z0 = torch.distributions.MultivariateNormal(
                loc=torch.zeros(15+augmented_dim).to(device),
                covariance_matrix=torch.eye(15+augmented_dim).to(device)
            )
        ...
        x_noise=[item[0] for item in samples]
        logpx=[item[1] for item in samples]

        # get 0~15 elements of x_noise as x
        x = [item[:, :15] for item in x_noise]
